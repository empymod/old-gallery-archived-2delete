...
pmodel = emg3d.utils.Model(pgrid, res_x=resh, res_y=resv)

# ===> Swap src_y and src_z <===
src_new = [src[0], src[1], src[4], src[5], src[2], src[3]]

sfield = emg3d.utils.get_source_field(pgrid, src_new, freq, strength)
pfield = emg3d.solve(pgrid, pmodel, sfield, verb=3)

# ===> Swap ry and zrec <===
e3d_fs_x = emg3d.utils.get_receiver(pgrid, pfield.fx, (rx, zrec, ry))
plot_result_rel(epm_fs_x, e3d_fs_x, x, r'Diffusive Fullspace $E_x$',
                vmin=-12, vmax=-6, mode='abs')

# ===> Swap ry and zrec; 'y'->'z' <===
e3d_fs_y = emg3d.utils.get_receiver(pgrid, pfield.fz, (rx, zrec, ry))
plot_result_rel(epm_fs_y, e3d_fs_y, x, r'Diffusive Fullspace $E_y$',
                vmin=-12, vmax=-6, mode='abs')

# ===> Swap ry and zrec; 'z'->'y' <===
e3d_fs_z = emg3d.utils.get_receiver(pgrid, pfield.fy, (rx, zrec, ry))
plot_result_rel(epm_fs_z, e3d_fs_z, x, r'Diffusive Fullspace $E_z$',
                vmin=-12, vmax=-6, mode='abs')
...
pmodel = emg3d.utils.Model(pgrid, resv, resh, resh)
...
e3d_fs_x = emg3d.utils.get_receiver(pgrid, pfield.fz, (zrec, ry, rx))
plot_result_rel(epm_fs_x, e3d_fs_x, x, r'Diffusive Fullspace $E_x$',
                vmin=-12, vmax=-6, mode='abs')

# ===> Swap rx and zrec <===
e3d_fs_y = emg3d.utils.get_receiver(pgrid, pfield.fy, (zrec, ry, rx))
plot_result_rel(epm_fs_y, e3d_fs_y, x, r'Diffusive Fullspace $E_y$',
                vmin=-12, vmax=-6, mode='abs')

# ===> Swap rx and zrec; 'z'->'x' <===
e3d_fs_z = emg3d.utils.get_receiver(pgrid, pfield.fx, (zrec, ry, rx))
plot_result_rel(epm_fs_z, e3d_fs_z, x, r'Diffusive Fullspace $E_z$',
                vmin=-12, vmax=-6, mode='abs')
